...
The methods in the class are public(It is javascript!). However there is way to create private members.
...
Here any member declared inside the annonymous fucntion is accessible only within that same closure. Public member can be placed inside within `return`.
